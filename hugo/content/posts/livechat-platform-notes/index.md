---
title: "Live chat platforms — quick notes (3CX, ManyChat, Kommunicate, Chatwoot)"
date: 2022-09-06T10:00:00-04:00
description: Links and pointers collected while comparing live-chat and bot options for websites.
tags:
    - Live chat
    - Chatbot
    - 3CX
    - Customer support
    - Chatwoot
---

## 3CX

Notes (FR): configuration guide for the 3CX Live Chat and Talk plugin — see the official 3CX documentation for the current setup steps.

- [YouTube — Microsoft ChatBot (PowerBI)](https://www.youtube.com/watch?v=nWxguR5B5-s)

## ManyChat

- [Pricing](https://manychat.com/pricing)

## Kommunicate

- [Kommunicate](https://www.kommunicate.io)

## Open source

- [Chatwoot on GitHub](https://github.com/chatwoot/chatwoot)
